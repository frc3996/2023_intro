...
class Limelight:
    _enabled: CamMode
    _snapshots: SnapshotMode
    _light = LEDState.OFF
    _stream_mode = 0
    _active_pipeline = 0

    def __init__(self, nt=None, camera=CamMode.PROCESSED, light=False):
        """
        Creates an instance of a limelight

        Args:
            nt: Pass in a custom networktables table if your limelight is
                running pypro on a table other than `limelight`
    ...
    def campose(self) -> Tuple[Tuple[float, float, float], Tuple[float, float, float]]:
        """
        Results of a 3D solution position, 6 numbers: Translation(x,y,z) Rotation(pitch, yaw, roll)
        """
        return self.__nt.getNumber("campose", 0)

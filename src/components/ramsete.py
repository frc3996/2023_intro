...
class RamseteComponent:
    ...
    # and seconds. These are recommended, but may be changed if wished.
    kRamseteB = 2
    kRamseteZeta = 0.7
    ...
    def _setup_trajectory_display(self):
        ...
        # subtable = nt.getTable("SmartDashboard").getSubTable("Autonomous Mode")
        # flags = (
        #     ntcore.EventFlags..NotifyFlags.NEW
        #     | ntcore.NetworkTablesInstance.NotifyFlags.UPDATE
        #     | ntcore.NetworkTablesInstance.NotifyFlags.IMMEDIATE
        # )
        # subtable.addListener("selected", self._on_chooser_change, flags)
